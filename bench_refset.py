...
preprocessings_list = [
    ...
    # ("top_simple", [('haar', pp.Haar()), ('savgol', pp.SavitzkyGolay()), ("gaussian1", pp.Gaussian(order=1, sigma=2)), ("detrend", pp.Detrend()), ("msc", pp.MultiplicativeScatterCorrection(scale=False))]),
...
models = [
    # [
    # (regressors.Transformer(), {'batch_size': 30, 'epoch': 1000, 'verbose': 0, 'patience': 150, 'optimizer': 'adam', 'loss': 'mse', "min_lr": 1e-6, "max_lr": 1e-2, "cycle_length": 32}),
    # (regressors.Decon(), {'batch_size': 250, 'epoch': 20000, 'verbose': 0, 'patience': 600, 'optimizer': 'adam', 'loss': 'mse', "min_lr": 1e-6, "max_lr": 1e-3, "cycle_length": 256}),
    # (regressors.Transformer_LongRange(), {'batch_size': 30, 'epoch': 1000, 'verbose': 0, 'patience': 150, 'optimizer': 'adam', 'loss': 'mse', "min_lr": 1e-6, "max_lr": 1e-2, "cycle_length": 32}),
    # (regressors.Transformer_NIRS(), {'batch_size': 30, 'epoch': 1000, 'verbose': 0, 'patience': 150, 'optimizer': 'adam', 'loss': 'mse', "min_lr": 1e-6, "max_lr": 1e-2, "cycle_length": 32}),
    ...
    (regressors.Decon(), {'batch_size': 100, 'epoch': 20000, 'verbose': 0, 'patience': 400, 'optimizer': 'adam', 'loss': 'mse', "min_lr": 1e-6, "max_lr": 1e-2, "cycle_length": 256}),
    # ]
]
...
#     c = 5 + i * 5
#     models. append(
#         (regressors.ML_Regressor(NonlinearPLSRegressor, name=f"NL_RBF10_PLS_{c}"), {"n_components": c, "poly_degree": 10, "gamma": 0.1})
#     )
#     models. append(
#         (regressors.ML_Regressor(NonlinearPLSRegressor, name=f"NL_RBF20_PLS_{c}"), {"n_components": c, "poly_degree": 20, "gamma": 0.1})
#     )
#     models.append(
#         (regressors.ML_Regressor(LWPLS, name=f"LWPLS_{c}"), {"max_component_number": c, "lambda_in_similarity": 0.05})
#     )
