...
from tensorflow.keras.layers import Dense, Conv1D, Activation, SpatialDropout1D, BatchNormalization, Flatten, Dropout, \
    Input, MaxPool1D, SeparableConv1D, Add, GlobalAveragePooling1D, DepthwiseConv1D, MaxPooling1D, LayerNormalization, \
    MultiHeadAttention
...
def xception1D(meta):
    ...
    outputs = xception_exit_flow(
        xception_middle_flow(xception_entry_flow(inputs)))
...
def decon(meta):
    ...
    model.add(MaxPooling1D(pool_size=2, strides=2))
    model.add(BatchNormalization())
    model.add(SeparableConv1D(64, kernel_size=3, depth_multiplier=1,
...
class Auto_Save(Callback):
    ...
    def on_train_end(self, logs=None):
        if self.params['verbose'] == 2:
            print('\nSaved best {0:6.4f} at epoch'.format(
...
def get_keras_model(run_name, model_func, epochs, batch_size, X_test, y_test, *, verbose=2, seed=0):
    early_stop = EarlyStopping(
        monitor='val_loss', patience=400, verbose=0, mode='min')
